...
void eeprom_i2c_init(void)
{
	...
	pad_force_mux(SC_P_PMIC_I2C_SDA, 0, SC_PAD_CONFIG_OD_IN, SC_PAD_ISO_OFF);
	(void) pad_set_gp_28fdsoi(SC_PT, SC_P_PMIC_I2C_SDA,
				  SC_PAD_28FDSOI_DSE_18V_1MA, SC_PAD_28FDSOI_PS_PU);
	pad_force_mux(SC_P_PMIC_I2C_SCL, 0, SC_PAD_CONFIG_OD_IN, SC_PAD_ISO_OFF);
	(void) pad_set_gp_28fdsoi(SC_PT, SC_P_PMIC_I2C_SCL,
				  SC_PAD_28FDSOI_DSE_18V_1MA, SC_PAD_28FDSOI_PS_PU);

	/* Initialize the I2C used to communicate with the EEPROM */
	LPI2C_MasterGetDefaultConfig(&lpi2c_masterConfig);

	/* MEK board spec is for 1M baud for EEPROM I2C bus */
	...
	LPI2C_MasterInit(LPI2C_SC, &lpi2c_masterConfig, SC_24MHZ);
